...
class ColumnRole(str, Enum):
    """The role a column plays in QC analysis."""
    ID = "id"                  # respondent identifier
    NPS = "nps"                # 0-10 "how likely to recommend" score
    SCALE = "scale"            # matrix / Likert scale item (e.g. 1-5, 1-7)
    OPENEND = "openend"        # free-text answer
    DURATION = "duration"      # time spent answering (seconds)
    CATEGORICAL = "categorical"  # single-choice / demographic
    OTHER = "other"            # ignored by rules
...
def load_data(path: str) -> pd.DataFrame:
    """Read a survey export from .csv / .xlsx / .xls into a DataFrame."""
...
@dataclass
class Schema:
    """Column-role mapping for one survey dataset."""
    ...
    def role_of(self, col: str) -> ColumnRole:
        """Return the role assigned to a given column."""
        if col == self.id_col:
...
def guess_schema(df: pd.DataFrame) -> Schema:
    ...
    Heuristics, in priority order per column:
      1. name keyword for ID / NPS / DURATION
      2. text content        -> open-ended
      3. small numeric scale -> scale item
